...
function [efs, reqTime] = discreteBeamAnalysis(nel, toplot, solver, unitsize)
    ...
    L = 1*unitsize; % length of the beam - m
    A = (10^-4)*(unitsize^2); % cross section - m^2
    rho = 7850* (unitsize^-3); % mass per unit volume - kg/m^3
    E = 2.1e11 * (unitsize^-2); % Young's modulus - Pascals
    I = ((10^-8)/12)*(unitsize^4);  % Moment of inertia - m^4 
